        ...
        for i in range(1, self.maxL + 1):
            ...
            for bidx in range(n_remainings):
                ...
                if self.enc_src0.dim() == 4:
                    ...
                    #uh.append(
                    #    [self.uh0[_k][:,bidx,:].unsqueeze(1).repeat(
                    #        1, preb_sz, 1) for _k in range(len(wargs.fltr_windows))])
            enc_src = tc.cat(enc_src, dim=1)
            uh = tc.cat(uh, dim=1)
            #uh = [tc.cat(item, dim=1) for item in map(list,zip(*uh))]
            cnt_bp = (i >= 2)
            ...
            if len(del_batch_idx) < n_remainings:
                ...
                #for __k in range(len(wargs.fltr_windows)):
                #    self.uh0[__k] = self.uh0[__k][:, filter(
                #        lambda x: x not in del_batch_idx, range(n_remainings)), :]
